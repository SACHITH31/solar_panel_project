...
function init() {
  ...
  document.getElementById("mvOkBtn").addEventListener("click", async () => {
    ...
    const result = await fetchMonthlyMaxWatts(month, year);

    document.getElementById("monthLoader").style.display = "none";

    if (!result.length) {
      alert("No data available for selected month and year");
      return;
    }

    drawMonthlyMaxBarChart(result, month, year);
  });
  ...
  function showCO2Saved(totalKwh) {
    ...
    document.getElementById(
      "co2_saved"
    ).innerHTML = `🌱 CO₂ Saved : <strong>${co2.toFixed(0)} kg</strong>`;
  }
  ...
  function updateInverterHealth(data) {
    ...
    document.getElementById(
      "inverter-health"
    ).innerHTML = `🟢 Inverter Health : <strong>${(100 - maxDrop).toFixed(
      1
    )}%</strong>`;
  }
  ...
  function setHistoricalStatus() {
    document.getElementById(
      "status"
    ).innerHTML = `<span>📅 Historical Data</span>`;
  }
  ...
  async function fetchMonthlyMaxWatts(month, year) {
    ...
    const lastDayAllowed = isCurrentMonth
      ? today.getDate()
      : new Date(year, month, 0).getDate();

    const requests = [];

    for (let d = 1; d <= lastDayAllowed; d++) {
      ...
      const req = new Promise((resolve) => {
        ...
        query.send((res) => {
          if (res.isError()) {
            resolve(null); // ❌ missing sheet → ignore
            return;
          }
          ...
          let maxVal = 0;
          for (let i = 0; i < data.getNumberOfRows(); i++) {
            maxVal = Math.max(maxVal, data.getValue(i, 0));
          }

          resolve({
            label: `${d} ${new Date(year, month - 1, d).toLocaleString("en", {
              month: "short",
            })}`,
            value: maxVal,
          });
